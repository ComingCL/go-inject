...
func init() {
	// 我们在Graph.Objects()中依赖math.Rand，这给它一些随机性。
...
func TestProvideWithFields(t *testing.T) {
	...
	if !reflect.DeepEqual(err.Error(), "fields were specified on object *inject_test.TypeAnswerStruct when it was provided") {
...
func TestInjectInlineMissing(t *testing.T) {
	...
	const msg = `inline struct on field Inline in type *struct { Inline struct { B *inject_test.TypeNestedStruct "inject:\"\"" } "inject:\"\"" } required an explicit "inline" tag`
	if err.Error() != msg {
		t.Fatalf("expected:\n%s\nactual:\n%s", msg, err.Error())
	}
}
...
type TypeAForTestDeepInject struct {
	C *TypeCForTestDeepInject `inject:""`
}
...
type TypeCForTestDeepInject struct {
	B *TypeBForTestDeepInject `inject:""`
}
...
func TestForDeepInject(t *testing.T) {
	...
	if err = g.Provide(&inject.Object{
		...
		t.Fatal("failed to inject b")
	}
	if err = g.Provide(&inject.Object{
		Value: c,
	}); err != nil {
		t.Fatal("failed to inject c")
	}
	if err = g.Provide(&inject.Object{
		Value: d,
	}); err != nil {
	...
	if c.B != b {
		t.Fatal("c'B is not b")
	}
	if d.A.C == nil {
		t.Fatal("d.A.C is nil")
	}
	if d.A.C.B == nil {
